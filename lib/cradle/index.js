...
exports.initialize = function (appRef) {
	...
	app.create = {
		...
		batch: function (batch, next) {
			...
			batch.brewed = (new Date(batch.brewed)).toString() === "Invalid Date" ? Date.now() : new Date(batch.brewed).getTime();
		...
		datapoint: function (batch, datapoint, next) {
			...
			if (!batch) return new Error('missing batch id');
			
			// defaults
			datapoint._id = '0';
			datapoint.at = (new Date(datapoint.at)).toString() === "Invalid Date" ? Date.now() : new Date(datapoint.at).getTime();
			
			// validate
			e = require(datapoint, 'action', 'string');
			e = optional(datapoint, 'temp', 'number');
			e = optional(datapoint, 'ambient', 'number');
			e = optional(datapoint, 'gravity', 'string');
			e = optional(datapoint, 'to', 'string');
			e = optional(datapoint, 'in', 'string');
			e = optional(datapoint, 'notes', 'string');
			e = optional(datapoint, 'tasting', 'object');
