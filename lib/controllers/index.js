...
*/
var Target = function (device, probeIndex, interval, batch, key) {
	// properties
	this.device = device;
...
Target.prototype.check = function (next) {
	if (this.nextEdge < Date.now()) {
		...
		this.nextEdge += this.interval;
		// log
		this.device.read('temp/' + this.probeIndex, function (e, probeData) {
			if (e) return next(e);
			db.get(that.batch, function (e, batch) {
				...
				point[that.key] = probeData.temp / 10;
				...
				db.save(batch._id, batch._rev, batch, function (e) {
					...
					app.log.info('[BCS] log : batch #' + batch.number + ' : ' + that.key + ' : ' + (probeData.temp / 10) + 'F : next tick at ' + new Date(that.nextEdge).toString());
					next();
				});
			});
		});
...
exports.initialize = function (appRef, next) {
	...
	db.view('bcs-controllers/byName', { include_docs: true }, function (e, rows) {
		...
		async.forEach(controllers, function (bcs, next) {
			bcs.device = new Device(bcs.host, bcs.port, function (e, deviceInfo) {
				if (e) return next(e);
				if (deviceInfo.ready) {
					// get probe information
					bcs.probes = [0,1,2,3];
					async.map(bcs.probes, function (i, next) {
						bcs.device.read('temp/' + i, next);
					}, function (e, probes) {
						if (e) return next(e);
						bcs.probes = probes;
						next();
